--2 PLAYER WIZARD TYCOON VERSION
_G.flykey = "V"
_G.remote = nil
_G.oldremote = nil
_G.debounced = false
...
Window:Tag({
    Title = "Ver - 2 Player Wizard Tycoon",
    Icon = "tree-palm",
    Color = BrickColor.new("Eggplant").Color,
    Radius = 0, -- from 0 to 13
})
...
function Kill(v)
  local contents = {
  [1] = CFrame.lookAt(v.Character.Head.Position, v.Character.Head.Position),
  [2] = 999,
  [3] = 999,
  [4] = game.Workspace.spizzarhs.Wand,
  [5] = 999,
  [6] = game.Workspace.spizzarhs
}
  for i = 1,100 do
    local remote = game.Workspace.spizzarhs.Wand.Fire
  remote:FireServer(table.unpack(contents))
  end

end

local S1 = Window:Section({
    Title = "2 Player Tycoon Cheats",
    Icon = "bird",
    Opened = true,
})
...
iter(game.Players:GetPlayers(), function(i, v)
    if v.Name ~= selfgetplayer().Name then
        local c2 = main:Section({
...
main:Button({
    Title = "Delete Player Gates",
    Callback = function(state)
      
        iter(game.Workspace:GetDescendants(), function(_, v)
            if v.Name == "Gate" then
                v:Destroy()
            end
        end)
...
main:Button({
    Title = "Crash Server",
    Desc = "spam a bit for it to work eventually itll kick everyone",
    Callback = function(state)
        for i = 1,50000 do
            wait()
            iter(game.Players:GetPlayers(), function(i,v)
